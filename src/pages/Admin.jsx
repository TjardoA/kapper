...
const tabs = [
  ...
  { key: "content", label: "Algemene info", icon: "⚙️" },
];
...
export default function Admin() {
  ...
  const submitGallery = async (e) => {
    ...
    const { error: err } = await addGalleryImage(galleryInput.trim(), gallery.length);
    if (err) return setError(err.message);
    setGalleryInput("");
    loadAll();
  };
            ...
            {tabs.map((t) => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left ${
                  tab === t.key ? "bg-white text-[#0f172a] font-semibold" : "hover:bg-white/10"
                }`}
              >
        ...
        <main className="flex-1 px-6 sm:px-8 py-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-500">Overzicht van jouw kapsalon</p>
            </div>
          </div>
        ...
        {tab === "opening" && (
          ...
          {tab === "content" && (
            <div className="space-y-6">
                ...
                <div className="divide-y mt-4">
                  {reviews.map((r) => (
                    <div key={r.id} className="py-3 flex items-start justify-between gap-3">
                      ...
                      <button className="text-red-600 text-sm" onClick={() => handleReviewDelete(r.id)}>
                        Verwijder
                      </button>
                    </div>
                  ))}
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                <Card title="Gallery">
                  <form className="flex gap-3 mb-4" onSubmit={submitGallery}>
                    <input
                      className="input flex-1"
                      placeholder="Afbeelding URL"
                      value={galleryInput}
